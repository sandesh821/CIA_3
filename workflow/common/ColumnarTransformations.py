...
class ColumnarTransformations:
    ...
    def _filter(self, df, column, transformValue, operator):
        local_df = df.copy()
        if operator=='In': 
            local_df =  local_df[local_df[column].isin(transformValue)]
        elif operator=='Not In':
            local_df =  local_df[~local_df[column].isin(transformValue)]
        return local_df

    def _scale(self, df, column, transformValue, operator):
        val  = float(transformValue)
        local_df = df[[column]]
        if operator=='Multiply':
            local_df[column] = local_df[column].apply(lambda i : i*val)
    ...
    def _upper_capping(self, df, column, transformValue, operator):
        local_df = df[[column]]
        local_df[local_df[column]>= float(operator)] = float(transformValue)
        return local_df[column]
    ...
    def _lower_capping(self, df, column, transformValue, operator):
        ...
        if transformValue == 'NA' :
            local_df[local_df[column]<=float(operator)] = nan
        else : 
            local_df[local_df[column]<=float(operator)] = transformValue
        return local_df[column]

    def _replace(self, df, column, transformValue, operator):
        local_df = df[[column]]
        local_df[column] = local_df[column].replace( operator, float(transformValue))
        return local_df[column]
    ...
    def perform_transformations(self, column_transform_operator_dict):
        ...
        for file_id, details in column_transform_operator_dict["file_details"].items():
            ...
            if details['file_type']=="FutureCovariates":
            ...
            for column_name, fields in details['columns'].items():
                for transformation_type, transformValue, operator in fields['transformations']:
                    print(transformation_type,operator)
                    if transformation_type == 'filter' :
                        transformValue = ast.literal_eval(transformValue)
                        df = eval(f"self._{transformation_type}")(df, column_name, transformValue, operator)
                    else : 
