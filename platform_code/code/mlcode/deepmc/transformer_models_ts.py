...
class GLU(tf.keras.layers.Layer):
    ...
    def call(self, x):
        return tf.math.multiply(self.linear_left(x), tf.keras.activations.sigmoid(self.linear_right(x)))
...
class FFT(tf.keras.layers.Layer):
    def __init__(self, time_step, order, output_channel):
        ...
        for i in range(3):
            if i == 0:
                self.GLUs.append(GLU(self.time_step * self.order, self.time_step * self.output_channel))
                self.GLUs.append(GLU(self.time_step * self.order, self.time_step * self.output_channel))
            elif i == 1:
