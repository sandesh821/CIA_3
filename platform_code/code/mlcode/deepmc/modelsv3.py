...
from keras.layers import Conv1D, Input, concatenate, Dropout, BatchNormalization, Reshape, Permute
...
def cnnlstm_layers(train_X,kernel_size=4):
    ...
    conv2 = BatchNormalization()(conv2)
    #conv3 = Conv1D(16,kernel_size,activation='relu')(conv2)
    lstm1 = LSTM(4, activation='relu', return_sequences=True,recurrent_dropout=0.4,dropout=0.4)(conv2)
    
    #flat1 = Flatten()(lstm1)

    #lstm1 = LSTM(100, activation='relu', return_sequences=True)(flat1)
...
def deepmc_pred_model(train_X, train_y, validation_data=None, server_mode=False):
    ...
    merge_cnn = concatenate(flat, axis=2)
    ##Attention Mechanism    
    merge1 = concatenate(flat, axis=2)
    attention1 = TimeDistributed(Dense(1, activation='relu'))(merge1)
    attention1 = Permute((2,1))(attention1)
    attention1 = TimeDistributed(Dense(10, activation='relu'))(attention1)
    attention1 = Permute((2,1))(attention1)
    ...
    merge2 = flat[-1]    
    dense_lstm = TimeDistributed(Dense(1, activation='relu'))(merge2)
    dense_lstm = Permute((2,1))(dense_lstm)
    dense_lstm = TimeDistributed(Dense(10, activation='relu'))(dense_lstm)
    dense_lstm = Permute((2,1))(dense_lstm)
    
    merge3 = concatenate([attention1, dense_lstm], axis=2)
    
    
    #attention = TimeDistributed(Dense(1, activation='relu'))(merge)
...
#     output = Dense(1)(hidden1)
    ...
    ##Decoder
    #repeat1 = RepeatVector(n_outputs)(merge)
    lstm2 = LSTM(4, activation='relu', return_sequences=True)(merge3)
    dense0 = TimeDistributed(Dense(1, activation='relu'))(lstm2)
    dense0 = Permute((2,1))(dense0)
    #lstm2 = RepeatVector(n_outputs)(lstm2)
    dense1 = TimeDistributed(Dense(8, activation='relu'))(dense0)
    #dropout1 = Dropout(0.2)(dense1)
    dense2 = TimeDistributed(Dense(n_outputs))(dense1)
    output = Permute((2,1))(dense2)
...
def deepmc_fit_model(model, train_X, train_y, validation_data=None, server_mode=False):
    batch_size = 32
        # fit network
    history = model.fit(train_X, train_y, epochs=20, batch_size=batch_size, validation_data=validation_data,verbose=1)
    # plot history
    if server_mode is False:
        plt.plot(history.history['loss'], label='train')
        if validation_data is not None:
