...
from tensorflow.keras.layers import Conv1D, Input, concatenate, Dropout, BatchNormalization, Reshape, Permute, LocallyConnected1D
from statsmodels.tsa.arima_model import ARIMA
from tensorflow.keras.models import load_model
from tensorflow.keras.models import Model
from tensorflow.keras.optimizers import Adam
from tensorflow.keras.metrics import RootMeanSquaredError
from deepmc.transformer import transformer_layer, deepmc_transformer_layer, hidden_transformer_layer
...
def dl_pred_model(train_X, train_y, validation_data=None, server_mode=False):
    ...
    #train_X = train_X.reshape(train_X.shape[0],train_X.shape[1],train_X.shape[2],1,1)
    #test_X = test_X.reshape(test_X.shape[0],test_X.shape[1],test_X.shape[2],1,1)
    #model.add(ConvLSTM2D(filters=100, kernel_size=(5,1), input_shape=(n_timesteps, n_features,1,1),activation='relu'))
    #model.add(Flatten())
...
def cnn_layers(train_X, kernel_size=4):
    ...
    conv1 = LocallyConnected1D(8,kernel_size,strides=1,activation='relu', kernel_initializer='he_normal')(in1)
    conv1 = BatchNormalization()(conv1)
    conv2 = LocallyConnected1D(8,kernel_size,strides=1,activation='relu', kernel_initializer='he_normal')(conv1)
    conv2 = BatchNormalization()(conv2)
    conv3 = LocallyConnected1D(8,kernel_size,strides=1,activation='relu', kernel_initializer='he_normal')(conv2)   
    return in1, conv3
...
def cnnlstm_layers(train_X,kernel_size=4):
    ...
    n_timesteps, n_features = train_X.shape[1], train_X.shape[2]
    in1 = Input(shape=(n_timesteps, n_features,))
    conv1 = LocallyConnected1D(4,kernel_size,activation='relu', kernel_initializer='he_normal')(in1)
    conv1 = BatchNormalization()(conv1)
    conv2 = LocallyConnected1D(8,kernel_size,activation='relu', kernel_initializer='he_normal')(conv1)
    conv2 = BatchNormalization()(conv2)
    lstm1 = LSTM(16, activation='relu', return_sequences=False,recurrent_dropout=0.2,dropout=0.2)(conv2)
...
def moddeepmc_pred_model(train_X, train_y, validation_data=None, server_mode=False, params={}):
    ...
    t_in = Input(shape=(train_X[k].shape[1], train_X[k].shape[2],))
    conv1 = LocallyConnected1D(6,kernel_size[k],strides=1,activation = params.get("activation"), kernel_initializer='he_normal')(t_in)
    conv1 = BatchNormalization()(conv1)
    ...
    lstm2 = LSTM(4, activation=params.get("activation"), return_sequences=True)(repeat1)
    dense1 = TimeDistributed(Dense(16, activation=params.get("activation")))(lstm2) 
    output = TimeDistributed(Dense(1))(dense1)
    
    ##Model Creation
    model = Model(inputs=inputs, outputs=output)
    print(params.get("loss"))
    model.compile( optimizer=Adam(learning_rate=params.get("learning_rate")), loss = params.get("loss"), metrics=[RootMeanSquaredError()])
    return model
    
def deepmc_fit_model(model, train_X, train_y, validation_data=None, server_mode=False,  epochs=30):
    batch_size = 8
        # fit network
...
def deepmc_wextmodel_pred_model(train_X, train_y, ext_model_input, validation_data=None, server_mode=False):
    ...
    model = Model(inputs=inputs, outputs=output)
    #plot_model(model, to_file='ws_cnnlstm_arima_arch.png',show_shapes=True)
    model.compile(loss='mse', optimizer='adam')
    # fit network
...
#     output = Dense(1)(hidden1)
    ...
    ##Model Creation
    model = Model(inputs=inputs, outputs=output)
    #plot_model(model, to_file='results/ws_cnnlstm_arch.png',show_shapes=True)
    model.compile(loss='mse', optimizer='adam')
    # fit network
...
def cnn_vanila_pred_model(train_X, train_y, validation_data=None, server_mode=False):
    ...
    model = Model(inputs=inputs, outputs=output)
    #plot_model(model, to_file='results/ws_arch6.png',show_shapes=True)
    model.compile(loss='mse', optimizer='adam')
    # fit network
    history = model.fit(train_X, train_y, epochs=30, batch_size=batch_size, validation_data=validation_data,verbose=1)
    # plot history
